...
function hienDanhSach(mang) {
    ...
    for (let i in mang.reverse()) {
        console.log(mang)
        danhSachUngHo += `
        <h3>${+i + 1}. ${mang[i].ten}</h3>
        <h4>Số điện thoại: ${mang[i].sodt}</h4>
        <h4>Số tiền: ${mang[i].sotien.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</h4>
